...
Este diretório contém o núcleo conversacional do bot OmniConnect, construído utilizando o **LangGraph**.

## O que deve estar aqui:
- **`state.ts`**: Definição do esquema de estado (`OmniState`) usando `Annotation` (inclui histórico de mensagens, dados de triagem coletados, `whatsappNumber` e intent atual).
